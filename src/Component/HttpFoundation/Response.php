...
{		//$this->response->minimize();
	...
	public function setOutput($mData)
	{
		$this->output .= $mData;
	}
	...
	{
		$this->output = preg_replace(array('/\>[^\S ]+/s','/[^\S ]+\</s','/(\s)+/s'),array('>','<','\\1'),$this->output);
	}
	...
	public function getResponseCodeText($code)
	{
	    switch ($code) {
	        ...
	        default:
	            throw new \RuntimeException('Unknown http status code "' . htmlentities($code) . '"');
	            break;
	    }
